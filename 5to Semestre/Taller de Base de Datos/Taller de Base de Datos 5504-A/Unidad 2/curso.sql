...
-- 2.
select max(stock_disponible) as Cantidad, descripcion from productos
 group by descripcion;
-- 3.
select descripcion, min(stock_disponible) from productos group by descripcion;
-- 4. Nombre del cliente, localidad
select nombre, localidad from clientes where localidad like 'MADRID';
-- 5. nombre del empleado, nombre del director
select apellido, director from empleados where director is not null;
-- 6. cantidad y nombre de director
select count(*) as Cantidad, director from empleados where director is not null group by director;
-- 7. fecha tal, cantidad
-- 8. numero del pedido
-- 9. numero del pedido, cantidad del pedido
